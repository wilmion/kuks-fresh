...
export class ScheduleComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  setRangeEnd(): void {
    ...
    range_end.style.left = `${
      this.scalableSize * (this.informationShedule.deliveryOff * 2)
    }%`;
  }
  ...
  setGraphic(): void {
    ...
    range.style.width = `${
      this.scalableSize * (Number(input_right.value) - Number(input_left.value))
    }%`;
  }
  ...
  control(e: any, direction: 'left' | 'right'): void {
    ...
    const parent: HTMLCollection = element.parentNode
  ...
  beforeControl(direction: 'left' | 'right'): void {
    ...
    const childs: NodeListOf<HTMLInputElement> =
      c_left.parentNode.parentNode.children;
  ...
  setTotalMount(): void {
    const total: string = (
      this.informationShedule.hourlyRate * this.informationShedule.totalHours -
      this.informationShedule.repeatWeekly * this.informationShedule.totalHours
    ).toFixed(2);
